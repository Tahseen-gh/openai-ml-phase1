...
def _heading_sections(text: str) -> List[Tuple[int, int, Optional[str]]]:
    matches = list(_HEADING_RE.finditer(text))
    sections: List[Tuple[int, int, Optional[str]]] = []
    if not matches:
...
def _label_for_offset(
    sections: List[Tuple[int, int, Optional[str]]], offset: int
) -> Optional[str]:
...
def _heading_for_chunk(
    sections: List[Tuple[int, int, Optional[str]]], start: int, end: int
) -> Optional[str]:
...
def chunk_text(
    doc_id: str, text: str, *, max_chars: int = 800, overlap: int = 120
) -> List[Chunk]:
    if max_chars <= 0:
    ...
    def flush():
        ...
        if not body:
            buf, buf_chars = [], 0
            return
        s0, eN = buf[0][0], buf[-1][1]
        heading = _heading_for_chunk(sections, s0, eN)
        chunks.append(
            Chunk(doc_id=doc_id, text=body, start=s0, end=eN, heading=heading)
        )
        ...
        if span_len > max_chars:
            ...
            while i < e:
                ...
                if body:
                    ...
                    chunks.append(
                        Chunk(doc_id=doc_id, text=body, start=i, end=j, heading=heading)
                    )
                if j >= e:
                    break
